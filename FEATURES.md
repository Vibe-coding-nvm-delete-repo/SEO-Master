...
| 2026-04-01 | Keyboard shortcuts: `Shift+1` now works directly from the Pages search/filter inputs and Token Management `auto-merge` filter controls that define the current visible list, while arbitrary editors remain protected. `Tab` no longer hijacks focus navigation while typing in editable fields. |
